...
def angle_calculator(xyz_distance):
    ...
    # calculate angle
    angle_xy_z = np.zeros(np.shape(x_component))
    angle_xy = np.zeros(np.shape(x_component))
    for i in range(len(x_component)):
        for j in range(len(x_component[i])):
            for k in range(len(x_component[i,j])):
                if x_component[i,j,k] == 0 and xy_projection[i,j,k] != 0:
                    angle_xy[i,j,k] = np.pi/2
                    angle_xy_z[i,j,k] = np.arctan(abs(z_component[i,j,k]/xy_projection[i,j,k]))
                elif x_component[i,j,k] != 0 and xy_projection[i,j,k] == 0:
                    angle_xy[i,j,k] = np.arctan(abs((y_component[i,j,k])/(x_component[i,j,k])))
                    angle_xy_z[i,j,k] = np.pi/2
                elif xy_projection[i,j,k] == 0 and x_component[i,j,k] == 0:
                    angle_xy[i,j,k] = np.pi/2
                    angle_xy_z[i,j,k] = np.pi/2
                elif xy_projection[i,j,k] != 0 and x_component[i,j,k] != 0:
                    angle_xy_z[i,j,k] = np.arctan(abs(z_component[i,j,k]/xy_projection[i,j,k]))
                    angle_xy[i,j,k] = np.arctan(abs((y_component[i,j,k])/(x_component[i,j,k]))) # 2D angle; angle between x and y projection
